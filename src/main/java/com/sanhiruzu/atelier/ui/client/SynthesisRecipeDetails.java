...
final class SynthesisRecipeDetails {
    ...
    static void render(GuiGraphics graphics, Font font, SynthesisStationMenu menu, SynthesisStationLayout layout, SynthesisProfile profile) {
        renderHeader(graphics, font, menu, layout, profile);
        menu.currentPlan().ifPresent(plan -> {
            renderPreview(graphics, font, layout, plan);
            renderSynthesisBoard(graphics, font, layout, profile, plan);
        });
    }

    private static void renderHeader(GuiGraphics graphics, Font font, SynthesisStationMenu menu, SynthesisStationLayout layout, SynthesisProfile profile) {
        ...
        graphics.drawString(font, Component.literal("[RECIPE DETAILS]"), layout.detailName.x(), layout.detailName.y(), SynthesisScreenTheme.ACCENT, false);
        graphics.drawString(font, SynthesisStationText.fitWidth(font, name, layout.detailCategory.width()), layout.detailCategory.x(), layout.detailCategory.y(), crafted ? SynthesisScreenTheme.TEXT : SynthesisScreenTheme.MUTED, false);
        SynthesisStationDrawing.frame(graphics, layout.detailIcon, accent);
        graphics.renderFakeItem(stack, layout.detailIcon.x() + 5, layout.detailIcon.y() + 5);
    }
